...
import { Imagebox, PageLayoutId, Videobox, ScrollButtons, MediaCard } from "../../components";
...
export default function MovieId() {
  ...
  const { scrollRef, scroll, scrollB, scrollRefB, scrollRefC, scrollC } = useScroll();
              ...
              {cast.slice(0,25).map((ca)=> (
                <div className='me-3 text-white text-center' key={ca.id}>
                  <Link to={`/person/${ca.id}`}>
                    <image src={`https://image.tmdb.org/t/p/w500/${ca.profile_path}`}
                  ...
                  <p className='small text-secoundary'>{ca.character}</p>
                </div>
              ))}
              </div>
